\documentclass[
	...
	head_space=0pt,
	foot_space=0pt,
	open_blacket_pos=nibu_tentsuki,
]{jlreq}
...
\newcommand{\hmmimg}{}
\newcommand{\hmmname}{幻世「ザ・ワールド」}
\newcommand{\hmmnamer}{ざわーるど}
\newcommand{\hmmtype}{SC}
\newcommand{\hmmnode}{5}
\newcommand{\hmmcost}{3}
\newcommand{\hmmspellist}{十六夜咲夜}
\newcommand{\hmmrangea}{x}
\newcommand{\hmmranget}{w}
\newcommand{\hmmplace}{G}
\newcommand{\hmmtext}{%
	【世界呪符】 維持コスト(3)
	
	（自動α）〔このカード〕がアクティブ状態である場合、〔全てのプレイヤー〕
	はカードのプレイ、起動効果の使用、戦闘のいずれも行うことができない。
	
	（自動α）メンテナンスフェイズ開始時に〔全てのプレイヤー〕は [3] 支払うことが
	できる。いずれかのプレイヤーが支払った場合、〔このカード〕をスリープ状態にする。
	このコストは「十六夜咲夜」の霊力によって軽減できる。
	
	（自動γ）ターン開始時に〔このカード〕はアクティブ状態になる。
}
\newcommand{\hmmp}{-}
\newcommand{\hmmt}{-}
\newcommand{\hmmflavor}{}
...
\begin{tikzpicture}[x=1mm,y=1mm]
	...
	\path(O3)node[above right,text width=25\zw,align=left,inner ysep=0,outer ysep=0]
		{\hmsfont\setlength{\baselineskip}{1.2mm}\fontsize{1.2mm}{0pt}\selectfont%
			\hmfukuro{{\©}上海アリス幻樂団\\
			{\©}さーくる⑨, 2020, 2020\\[-1mm]
			CC-BY-NC 4.0}};
	\path(O1)|-(O3)node[pos=.75,above left,inner xsep=0,outer xsep=0,font=\hmsfont\small]
		{\hmfukuro{\hmmnumtype~}};
	\path(O1)|-(O3)node[pos=.75,above right,inner xsep=0,outer xsep=0,font=\hmsfont\small]
		{\hmfukuro{\hmmnum}};
	\path(O1)|-(O3)node[pos=.68,above,font=\hmsfont\small]{\hmfukuro{\hmmexpantion}};

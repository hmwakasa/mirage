...
\usepackage{luatexja,pdfrender,xparse,fontspec,bxokumacro,bxghost,pxrubrica}
...
\begin{tikzpicture}[x=1mm,y=1mm]
	...
	% \path(SC2)node[rangecolor]{\LARGE\strut\hmicot};
	% % 効果範囲
	% \filldraw[fill=rangebcolor,draw=rangebcolor!80,thick](GC2)circle[radius=2.5];
	% \path(GC2)node[rangecolor]{\LARGE\strut\hmicom};
	% 種族
...
\begin{tikzpicture}[x=1mm,y=1mm]
	...
	% 発動期間
	\filldraw[fill=rangebcolor,draw=rangebcolor!80,thick](SC2)circle[radius=2.5];
	\path(SC2)node[rangecolor]{\LARGE\strut\hmicom};
	% 効果範囲
	\filldraw[fill=rangebcolor,draw=rangebcolor!80,thick](GC2)circle[radius=2.5];
	\path(GC2)node[rangecolor]{\LARGE\strut\hmicot};
	% 種族
...
\begin{tikzpicture}[x=1mm,y=1mm]
	...
	\fill[fill=costcolor!70]
		...
		(C1)-|(C3)|-cycle;
	\path(C1)|-(C3)node[near end,above,outer ysep=0]{\hmcfont\small COST};
	\path(C1)--(C3)node[midway]{\hmcfont\Huge\strut\hmfukuro{0}};
	% ノード
	\filldraw[fill=nodecolor!30,draw=nodecolor,thick](N1)rectangle(N3);
	\fill[fill=nodecolor!70](N1)--(N3)--(N4)--(N2)--cycle;
	\path(N1)-|(N3)node[near start,below,outer ysep=0]{\hmcfont\small NODE};
	\path(N1)--(N3)node[midway]{\hmcfont\Huge\strut\hmfukuro{1}};
	% % 霊力
	% \filldraw[fill=spiritcolor!40,draw=spiritcolor,thick](SC)circle[radius=3];
	% \fill[fill=spiritcolor!10](SC)circle[radius=2];
	% \path(SC)--++(0,2.5)node{\small\hmgcfont\hmfukuro{\strut Spirit}};
	...
	% 発動期間
	\filldraw[fill=rangebcolor,draw=rangebcolor!80,thick](SC2)circle[radius=2.5];
	\path(SC2)node[rangecolor]{\LARGE\strut\hmicox};
	% 効果範囲
	\filldraw[fill=rangebcolor,draw=rangebcolor!80,thick](GC2)circle[radius=2.5];
	\path(GC2)node[rangecolor]{\LARGE\strut\hmicoe};
	% 種族
	\path(I3)-|(I1)node[midway,above left]{\large\hmsfont\hmfukuro{}};
	% テキスト
...
\begin{tikzpicture}[x=1mm,y=1mm]
	...
	\fill[fill=costcolor!70]
		...
		(C1)-|(C3)|-cycle;
	\path(C1)|-(C3)node[near end,above,outer ysep=0]{\hmcfont\small COST};
	\path(C1)--(C3)node[midway]{\hmcfont\Huge\strut\hmfukuro{2}};
	% ノード
	\filldraw[fill=nodecolor!30,draw=nodecolor,thick](N1)rectangle(N3);
	\fill[fill=nodecolor!70](N1)--(N3)--(N4)--(N2)--cycle;
	\path(N1)-|(N3)node[near start,below,outer ysep=0]{\hmcfont\small NODE};
	\path(N1)--(N3)node[midway]{\hmcfont\Huge\strut\hmfukuro{5}};
	% 霊力
	\filldraw[fill=spiritcolor!40,draw=spiritcolor,thick](SC)circle[radius=3];
	\fill[fill=spiritcolor!10](SC)circle[radius=2];
	\path(SC)--++(0,2.5)node{\small\hmgcfont\hmfukuro{\strut Spirit}};
...
\begin{tikzpicture}[x=1mm,y=1mm]
	...
	\fill[spellcolor](O1)-|(O3)-|cycle;
	\draw[path picture={\node at (path picture bounding box.center)
		{\includegraphics[height=88mm]{in_back.pdf}};}](O1)-|(O3)-|cycle;
	% イラスト
	\draw[path picture={\node at (path picture bounding box.center)
		{\includegraphics[width=49mm]{sumire.jpg}};}](I1)rectangle(I3);
	%\draw[path picture={\node at (path picture bounding box.center)
	...
	\fill[black,path fading=west](M1)rectangle(M3);
	\path(MT)node[below,inner sep=0,outer sep=0]
		{\hbox{\tate\hminwidth{48mm}{\fontsize{6mm}{0}\hmnfont\hmfukuro[.3pt]{オカルトボール}}}};
	\path(RT)node[below,white,inner sep=0,outer sep=0]
		{\hbox{\tate\hminwidth*{44mm}{\fontsize{2mm}{0}\hmnfont おかるとぼーる}}};
	% タイプ、術者
	\filldraw[fill=incidentcolor!50,draw=incidentcolor](TC)circle[radius=1.9];
	\draw(TC)node{\hmtfont{In}};
	\path(TC)--++(2,0)node[right]{\hmsfont\hmfukuro{}};
	% コスト
	\filldraw[fill=costcolor!30,draw=costcolor,thick](C1)rectangle(C3);
	\fill[fill=costcolor!70]
		...
		(C1)-|(C3)|-cycle;
	\path(C1)|-(C3)node[near end,above,outer ysep=0]{\hmcfont\small COST};
	\path(C1)--(C3)node[midway]{\hmcfont\Huge\strut\hmfukuro{-}};
	% ノード
	\filldraw[fill=nodecolor!30,draw=nodecolor,thick](N1)rectangle(N3);
	\fill[fill=nodecolor!70](N1)--(N3)--(N4)--(N2)--cycle;
	\path(N1)-|(N3)node[near start,below,outer ysep=0]{\hmcfont\small NODE};
	\path(N1)--(N3)node[midway]{\hmcfont\Huge\strut\hmfukuro{-}};
	% % 霊力
	% \filldraw[fill=spiritcolor!40,draw=spiritcolor,thick](SC)circle[radius=3];
	% \fill[fill=spiritcolor!10](SC)circle[radius=2];
	% \path(SC)--++(0,2.5)node{\small\hmgcfont\hmfukuro{\strut Spirit}};
	...
	\path(GC)node{\Huge\hmgfont 7};
	% 種族
	\path(I3)-|(I1)node[midway,above left]{\large\hmsfont\hmfukuro{}};
	% テキスト
	\draw[fill=icoDcolor!30,fill opacity=.85](B1)rectangle(B3);
	%% 場所
	\path(B1)--(B3)node[midway,text opacity=.3]{\fontsize{20mm}{0}\hmicoD};
	% %% テキスト
	% \path(B1)-|(B3)node[
	% 	midway,below right,outer sep=0,inner sep=1mm,text width=53mm,text justified,font=\small\sffamily]{
	% 		（プレイ）目標の〔キャラクター 1 枚〕に 3 ダメージを与え、スリープ状態にする。
	...
	\path(B1)-|(B3*)node[
		midway,below right,outer sep=0,inner sep=1mm,text width=53mm,text justified,font=\small\sffamily]{
			（自動α）〔全てのキャラクター〕は「変身」を持ち、このカードの変身状態を変身状態として持つ。
			
			（収集）〔全てのキャラクター〕は、決死状態になった場合収集される。
		};
	%% 戦闘力
	\path($(B1)-(7,0)$)|-(B3*)node
		[midway,above,outer sep=0,inner xsep=0,inner ysep=1mm,text width=14mm,text centered,font=\LARGE\hmpfont]
	...
	\path(B3*)node[below right,inner sep=0,outer sep=0]{\rotatebox{180}{%
		\begin{tikzpicture}[x=1mm,y=1mm]
			...
			\path($(Q1)-(7,0)$)|-(Q3)node
				...
				{\mbox{\makebox[4mm][r]{10}/\makebox[4mm][l]{10}}};
